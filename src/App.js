...
class App extends Component {
  ...
  getInfo(){
    ...
    if(typeof web3.eth !== 'undefined'){
      IGBookContract = web3.eth.contract(IGBookABI).at(IGBookAddress);
      if(typeof web3.eth.accounts !== 'undefined') {
        if(typeof web3.eth.accounts[0] !== 'undefined'){
          ...
          IGBookContract.getUsers(function(err,usersResult){
            if(err) {
              outerThis.setState({
                users: {}
              });
              ...
              _.map(usersResult,function(key){
                ...
                IGBookContract.getUser(key,function(err,userResult){
      ...
      if(typeof web3.eth.accounts !== 'undefined') {
        if(typeof web3.eth.accounts[0] !== 'undefined'){
          ...
          IGBookContract.getUser(key,function(err,userResult){
            ...
            IGBookContract.getUserImages(key,function(err,imagesResult){
              if(err) {
                ...
                _.map(imagesResult,function(imageHash){
                  IGBookContract.getImage(imageHash,function(err,imageResult){
  ...
  addImageToUser() { // handles the "add image to user" button functionality
    ...
    try {
      IGBookContract.addImageToUser.estimateGas(outerThis.state.newImageURL,"0x"+outerThis.state.newImageSHA,{from:defaultAccount}, function(err, result){
        if(err) {
          throw err;
        } else {
          console.log("addNewImage gas estimate : " + result);
          var myGasNum = result;
          IGBookContract.addImageToUser.sendTransaction(outerThis.state.newImageURL,"0x"+outerThis.state.newImageSHA,{from:defaultAccount, gas: myGasNum}, function(err, result){
            if(err) {
              throw err;
            } else {
  ...
  signGuestbook() { // handles the "sign guestbook now" button functionality
    ...
    try {
      IGBookContract.registerNewUser.estimateGas(outerThis.state.defaultHandle,outerThis.state.defaultCity,outerThis.state.defaultState,outerThis.state.defaultCountry,{from:defaultAccount}, function(err, result){
        if(err) {
          throw err;
        } else {
          console.log("registerNewUser gas estimate : " + result);
          var myGasNum = result;
          IGBookContract.registerNewUser.sendTransaction(outerThis.state.defaultHandle,outerThis.state.defaultCity,outerThis.state.defaultState,outerThis.state.defaultCountry,{from:defaultAccount, gas: myGasNum}, function(err, result){
            if(err) {
              throw err;
            } else {
